...
from ..services.pdf_report_service_reportlab import generate_report
from ..services.security_service import validate_deal_access, validate_org_ownership
...
async def get_report(
    ...
    # Extract deal_id from filename (assuming format like 'project-name-dealid-timestamp.pdf')
    # This is a simple implementation - in production, you would store report metadata in the database
    try:
        # Try to find a deal that has this report file associated with it
        deal = db.query(Deal).filter(Deal.report_file == file_path).first()
        if deal:
            # Validate organization access
            validate_org_ownership(current_user, deal)
        else:
            # If no deal is found with this report, only allow admins to access it
